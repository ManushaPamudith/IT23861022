...
test.describe('Assignment 1 Automation Suite', () => {
    ...
    // Global safety timeout (2 minutes)
    ...
    for (const tc of testCases) {
        test(`Test ${tc.id}`, async ({ page }) => {
            ...
            // OPTIMIZED WAIT: 5 seconds for normal, 10 for long.
            // This is much faster than the previous 20/40s.
            const waitTime = tc.id === 'Pos_Fun_0009' ? 10000 : 5000;
            ...
            if (tc.id.startsWith('Neg')) {
                ...
                // SMART WAIT: Waits UP TO 30s, but proceeds immediately if found.
                await expect(page.locator('body')).toContainText(tc.expected, { timeout: 30000 });
